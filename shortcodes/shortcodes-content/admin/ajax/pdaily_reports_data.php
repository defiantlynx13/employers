...
add_action("wp_ajax_pdaily_reports_data","pdaily_reports_data_server_side_callback");
add_action("wp_ajax_pdr_insert","pdr_insert_server_side_callback");
add_action("wp_ajax_pdr_delete","pdr_delete_server_side_callback");
add_action("wp_ajax_pdr_update","pdr_update_server_side_callback");


function pdaily_reports_data_server_side_callback()
{
...
//        $sub_array[] = '<button type="button" id="se_pdr_delete" name="se_pdr_delete" class="btn btn-sm btn-danger btn-rounded  mr-1 mb-1" data-id="'.$result->id.'"  ><i class="icon-android-delete fa-lg"></i>&nbsp;&nbsp;حذف</button><button type="button" id="se_pdr_complete" name="se_pdr_complete" class="btn btn-sm btn-primary btn-rounded  mr-1 mb-1" data-id="'.$result->id.'"><i class="icon-paper fa-lg"></i>تکمیل گزارش &nbsp;&nbsp;</button></button><button type="button" id="se_pdr_edit" name="se_pdr_edit" class="btn btn-sm btn-info btn-rounded  mr-1 mb-1" data-id="'.$result->id.'" data-timestamp="'.$temp->format('U').'" data-max_temp="'.$result -> max_temp.'" data-min_temp="'.$result -> min_temp.'" data-status="'.$result -> status.'" data-date="'.jdate("Y/m/d", strtotime($result ->date)).'">&nbsp;&nbsp;<i class="icon-pencil3 fa-lg"></i>ویرایش</button>';
        $sub_array[] = '<button type="button" id="se_pdr_delete" name="se_pdr_delete" class="btn btn-sm btn-danger btn-rounded  mr-1 mb-1" data-id="'.$result->id.'"  ><i class="icon-android-delete fa-lg"></i>&nbsp;&nbsp;حذف</button><button type="button" id="se_pdr_complete" name="se_pdr_complete" class="btn btn-sm btn-primary btn-rounded  mr-1 mb-1" data-id="'.$result->id.'"><i class="icon-paper fa-lg"></i>تکمیل گزارش &nbsp;&nbsp;</button>';
...
function pdr_insert_server_side_callback()
{
    header("Content-Type: application/json");
    $request= $_GET;

    if (isset($request['p_id'],$request['pdr_date'],$request['pdr_min_temp'],$request['pdr_max_temp'],$request['pdr_status']))
    {
        ...
        $pdr_status=sanitize_text_field($request['pdr_status']);
...
function pdr_delete_server_side_callback()
{
    header("Content-Type: application/json");
    $request= $_GET;
...
function pdr_update_server_side_callback()
{
    header("Content-Type: application/json");
    $request= $_GET;

    if (isset($request['p_id'],$request['pdr_id_edit'],$request['pdr_date_edit'],$request['pdr_max_temp_edit'],$request['pdr_min_temp_edit'],$request['pdr_status_edit']))
    {
        ...
        $status=sanitize_text_field($request['pdr_status_edit']);
        require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
        global $wpdb;
        $table = $wpdb->prefix .'pdaily_reports';
