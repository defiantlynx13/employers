...
(function($) {
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			{
				inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('بارگزاری تصویر')).click(StartUpload));
				inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_start_upload').attr('type', 'button').attr('aria-label', Translate('بارگزاری تصویر')).click(StartUpload));

				inforow.addClass('ff_fileupload_queued');
			}

			inforow.find('.ff_fileupload_actions').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('حذف تصویر از لیست بارگزاری')).click(RemoveFile));
			inforow.find('.ff_fileupload_actions_mobile').append($('<button>').addClass('ff_fileupload_remove_file').attr('type', 'button').attr('aria-label', Translate('حذف تصویر از لیست بارگزاری')).click(RemoveFile));
		...
		var UploadFailed = function(e, data) {
			...
			{
				data.ff_info.fileinfo.text(FormatStr(Translate('{0} | خطای شبکه, در حال تلاش مجدد برای اپلود ({1})'), data.ff_info.displayfilesize, data.errorThrown));
			...
			{
				// Set the error info.
				if (data.errorThrown === 'abort')  data.ff_info.errors.push(Translate('اپلود تصویر کنسل شده است.'));
				else if (data.errorThrown === 'failed_with_msg')  data.ff_info.errors.push(FormatStr(Translate('اپلود با شکست مواجه شد.  {0} ({1})'), EscapeHTML(data.result.error), EscapeHTML(data.result.errorcode)));
				else  data.ff_info.errors.push(Translate('اپلود تصویر کنسل شده است..'));
				...
				// Alter remove buttons.
				data.ff_info.inforow.find('button.ff_fileupload_remove_file').attr('aria-label', Translate('حذف تصویر از لیست بارگزاری'));
			}
		};
		...
		var UploadDone = function(e, data) {
			...
			{
				if (typeof(data.result.error) !== 'string')  data.result.error = Translate('سرور قادر به اپلود تصویر نمی باشد.  اطلاعات بیشتری در دسترس نیست.');
			...
			{
				// Set the status.
				data.ff_info.inforow.find('.ff_fileupload_fileinfo').text(data.ff_info.displayfilesize + ' | ' + Translate('بارگزاری کامل شد'));
		...
		var UploadChunkSend = function(e, data) {
			if (data.ff_info)
			{
				...
				if (data.ff_info.lastresult && !data.ff_info.lastresult.success)
				{
					...
					if (typeof(data.ff_info.lastresult.error) !== 'string')  data.ff_info.lastresult.error = Translate('سرور قادر به اپلود تصویر نمی باشد.  اطلاعات بیشتری در دسترس نیست.');
